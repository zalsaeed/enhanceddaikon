...
//            newChild.checkForDerivedVariables(type, name, offset);
  ...
  protected void checkForDerivedVariables(Class<?> type, String theName, String offset) {
		   ...
		   Class<?> parentType)
   {
       ...
       if (implementsList(type)) {
           ...
           for (Object obj:listElements){
               ...
               /*
                * When an element is null then you can't get any information from (e.g. the hashCode). And that makes
                * it useless as well for invariants detection. Thus, we are going to ignore any null element
                */
             // Fix for elements that are null, a bug discovered when ran test generated by Randoop.
               if(obj == null)
                 continue;
               ...
               System.out.println("\t\t\t\t[Chicory.DaikonVariableInfo.checkForListDecl()] name: " + obj.getClass().getSimpleName());
   ...
   protected List<Object> getListElements(Object obj, String listName){
	   ...
	   // TODO remove this print stmt:
       System.out.println("\t\t\t\t\t[Chicory.DaikonVariableInfo.getListElements()] obj: " + obj.getClass().getName());
     System.out.println("\t\t\t\t\t[Chicory.DaikonVariableInfo.getListElements()] listName: " + listName);
