  ...
  String varname = var().csharp_name();
    ...
    if (is_type()) {
        ...
        result = (str == null || "null".equals(str))
               ? (varname + " == null")
               : (varname + ".Equals(" + ((str==null) ? "null" : "\"" + UtilMDE.escapeNonASCII(str) + "\"") + ")");
    } else {
        List<String> names = new ArrayList<String>();
        for (int i=0; i<num_elts; i++) {
            String str = elts[i];
            names.add((str == null) ? "null" : ((str==null) ? "null" : "\"" + UtilMDE.escapeNonASCII(str) + "\""));
        }
    ...
    if (status == InvariantStatus.FALSIFIED) {
      if (logOn() && mutate) {
        ...
        log ("destroyed on sample %s previous vals = {%s} num_elts = %s",
             ((v==null) ? "null" : "\"" + UtilMDE.escapeNonASCII(v) + "\""), eltString, num_elts);
      }
